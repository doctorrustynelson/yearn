...
manager.init = function( orgs, yearn_utils ){
	...
	Object.keys( orgs ).forEach( function( org ){
			...
			switch(path.sep){
				case '/':
					wild_path = new RegExp( '' + 
						path.resolve( orgs[ org ] ).replace( /\*/, '([^/]+)' ).replace( /\*/g, '\1' ) + 
						'/?([^/]*)/([^/]*)/(.*)' );
					break;
				case '\\':
					wild_path = new RegExp( '' + 
						path.resolve( orgs[ org ] ).replace( /\\/g, '\\\\').replace( /\*/, '([^\\\\]+)' ).replace( /\*/g, '\1' ) + 
						'\\\\?([^\\\\]*)\\\\([^\\\\]*)\\\\(.*)' );
					break;
				default: 
		...
		if( orgs[ org ] === './node_modules' ){
			...
			switch(path.sep){
				case '/':
					resolved_path = new RegExp( '(' + path.resolve( orgs[ org ] ) + ')/?([^/]*)/([^/]*)/(.*)' );
					break;
				case '\\':
					resolved_path = new RegExp( '(' + path.resolve( orgs[ org ] ).replace( /\\/g, '\\\\') + ')\\\\?([^\\\\]*)\\\\([^\\\\]*)\\\\(.*)' );
					break;
				default: 
					throw new Error( 'Unrecognized path seperator: ' + path.sep );	
			}
...
manager.populate = function( parent_path, parent_id, package_json_location ){
	...
	var pkg_contents = JSON5.parse( fs.readFileSync( package_json_location, 'utf8' ));
