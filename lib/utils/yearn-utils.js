...
module.exports = function( config ){
	...
	yutils.setLOGGER = function( type ){
		yutils.LOGGER = require( './logger' ).getLOGGER( type, 'ynpm' );
	};
	...
	yutils.extractYearningParts = function( path ){
		...
		case 1:
			yutils.LOGGER.debug( 'Yearning path ("' + path + '") is of the form "module".' );
		...
		case 2:
			...
			if( valid_org && !valid_sem_ver ){
				yutils.LOGGER.debug( 'Yearning path ("' + path + '") is of the form "org/module".' );
				return { org: spliced[0], module: spliced[1], version: undefined };
			} else if( !valid_org && valid_sem_ver ) {

				yutils.LOGGER.debug( 'Yearning path ("' + path + '") is of the form "module/version".' );
				return { org: '', module: spliced[0], version: spliced[1] };
			}
				
			yutils.LOGGER.error( 'Yearning path ("' + path + '") could not be interperted.' );
			return null;
		case 3:
			yutils.LOGGER.debug( 'Yearning path ("' + path + '") is of the form "org/module/version".' );
