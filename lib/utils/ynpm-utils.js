...
var LOGGER = require( './logger', 'ynpm' );

module.exports = {

	createTempDir: function(){
		// TODO: add temp module 
	},

	npmInstallToDir: function( dest_root ){
		// TODO:
		dest_root = dest_root;
	},
	...
	translateLegacyDependencyStructure: function( src_root, dest_root ){
		...
		if( fs.existsSync( path.join( src_root, 'node_modules' ) ) ) {
			...
			dependencies.forEach( function( dependency ){
				if( !this.translateLegacyDependencyStructure( path.join( src_root, 'node_modules', dependency ) ) )
					return false;
			} );
		}

		if( !fs.existsSync( path.join( src_root, 'package.json' ) ) ) {
			LOGGER.error( src_root + ' does not contain a package.json.' );
			return false;
		}
		...
		if( !fs.existsSync( path.join( dest_root, name ) ) ) {
			...
			LOGGER.debug( 'A version of ' + name + ' was previously installed.' );
		}

		if( fs.existsSync( path.join( dest_root, name, version ) ) ) {
			LOGGER.debug( 'Module ' + name + ' v.' + version + ' was already installed.' );
		} else {
			LOGGER.debug( 'Module ' + name + ' v.' + version + ' being installed.' );
			fs.mkdirSync( path.join( dest_root, name, version ) );

			fs.readdirSync( src_root ).foreach( function( file ){
