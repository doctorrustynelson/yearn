...
module.exports = function( config ){
	...
	ynpm_utils.translateLegacyDependencyStructure = function( src_root, primary_org, primary_module, secondary_org, no_alias ){
		...
		if( !fs.existsSync( path.join( src_root, 'package.json' ) ) ) {
			// src_root does not contain a package.json and thus is not a module.
			return true;
		}
		...
		if( !fs.existsSync( path.join( dest_primary_root, desired.module ) ) ) {
			console.log( 'No version of ' + desired.module + ' was previously installed.' );
			fs.mkdirsSync( path.join( dest_primary_root, desired.module ) );
		} else {
			console.warn( 'A version of ' + desired.module + ' was previously installed.' );
		}

		if( fs.existsSync( path.join( dest_primary_root, desired.module, desired.version ) ) ) {
			console.warn( 'Module ' + desired.module + ' v.' + desired.version + ' was already installed.' );
		} else {
			console.log( 'Module ' + desired.module + ' v.' + desired.version + ' being installed.' );
