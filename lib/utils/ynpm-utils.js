...
var ynpm_utils = module.exports = {
	...
	npmInstallToDir: function( module, dest, callback ){
		npm.load( function( ){
			npm.commands.install( dest, module, callback );
		});
	},
	
	npmInstallToDirSync: function( module, dest ){
		var done = false;
		var error = false;
	...
	translateLegacyDependencyStructure: function( src_root, dest_primary_root, dest_secondary_root ){
		...
		if( fs.existsSync( path.join( src_root, 'node_modules' ) ) ) {
			...
			dependencies.forEach( function( dependency ){
				if( !ynpm_utils.translateLegacyDependencyStructure( path.join( src_root, 'node_modules', dependency ), dest_secondary_root, dest_secondary_root ) )
					return false;
			} );
		}
		...
		if( !fs.existsSync( path.join( dest_primary_root, name ) ) ) {
			...
			fs.mkdirSync( path.join( dest_primary_root, name ) );
		} else {
		...
		if( fs.existsSync( path.join( dest_primary_root, name, version ) ) ) {
			...
			fs.mkdirSync( path.join( dest_primary_root, name, version ) );
