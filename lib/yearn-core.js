...
core.determineYearningPath = function( desired, parent ){
	...
	if( desired.version === undefined ){
		...
		if( desired.org === '' && core.package_dependencies[ package_json_location ][ desired.module ] ){
		...
		} else if( desired.org !== '' && core.package_dependencies[ package_json_location ][ desired.org + config.delimiters.org + desired.module ] ){
			yutils.LOGGER.debug( 'Resolving version for ' + yutils.constructYearningString( desired ) + '.' );
