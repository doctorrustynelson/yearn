...
function extractCravingParts( path ){
	var spliced = path.split( '/' );
	
	switch( spliced.length ){
		case 1:
			LOGGER.debug( 'Craving path ("' + path + '") is of the form "module".' );
		...
		case 2:
			...
			if( valid_org && !valid_sem_ver ){
				...
				LOGGER.debug( 'Craving path ("' + path + '") is of the form "org/module".' );
				return { org: spliced[0], module: spliced[1], version: undefined };
			} else if( !valid_org && valid_sem_ver ) {

				LOGGER.debug( 'Craving path ("' + path + '") is of the form "module/version".' );
				return { org: '', module: spliced[0], version: spliced[1] };
			}
			
			LOGGER.error( 'Craving path ("' + path + '") could not be interperted.' );
			return null;
		case 3:
			LOGGER.debug( 'Craving path ("' + path + '") is of the form "org/module/version".' );
...
function determineCravingPath( desired, parent ){
	var parent_location = ( fs.statSync( parent.id ).isDirectory() ? parent.id : path.dirname(parent.id) );
...
function yearn( desired ){
	...
	switch( typeof desired ){
		case 'object':
			...
			break;
		case 'string':
			if( isDirectCraving( desired ) ){
				LOGGER.debug( 'Direct craving.' );
				LOGGER.info( 'Satiating with [' + desired + '].' );
				return module.constructor._load( desired, this );
			} else {
				LOGGER.debug( 'Implicit craving.' );
				desired = extractCravingParts( desired );
			}
			break;
		default:
	...
	var path = determineCravingPath( desired, this );
