...
var fs = require( 'fs' );
var yutils = require( './yearn-utils' );
...
function determineYearningPath( desired, parent ){
	...
	/*
	TODO: fix for travelling up the tree
	function fixModuleLocation( module_root, desired ){
		if( fs.existsSync( path.join( module_root, desired.module ) ) ){
			return path.join( module_root, desired.module );
		} else {
			if ( desired.org === '' && orgs[ desired.org ] === './node_modules' ) {
				
				if( module_root.length < 3 || module_root.indexOf( 'node_modules' ) === -1 ){
					LOGGER.error( 'Module "' + desired.module + '" was not found in org "' + desired.org + '".' );
					return null;
				}
				
				LOGGER.debug( 'Searching up the Legacy Tree for valid modules' );
				fixModuleLocation( path.resolve( module_location, '../../node_modules' ), desired );
			} else {
				LOGGER.error( 'Module "' + desired.module + '" was not found in org "' + desired.org + '".' );
				return null;
			}
	...
	var module_location = fixModuleLocation( 
		path.resolve( path.dirname( package_json_location ), orgs[ desired.org ] ),
		desired
	);
	*/
...
module.exports = function( yearn_options ){
	...
	if( LOGGER === null)
		LOGGER = require( './logger' )( log_level );
