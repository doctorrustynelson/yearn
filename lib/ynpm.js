...
module.exports = function( config ){
	...
	ynpm.init = function( npm_config, callback ){
		...
		npm_config = require( './utils/config' )._merge( npm_config === null ? {} : npm_config, config.npmconfig );
		
		npm.load( npm_config, function( err, initialized_npm ){
			if( err === null ){
				nutils.LOGGER.debug( 'Initialized npm.' );
				npm = initialized_npm;
				nutils.setNPM( initialized_npm );
			} else {
	...
	ynpm.commands.check = function( desired, callback ){
		...
		nutils.getLatestVersionOf( desired.module, function( err, latest_version ){
			...
			if( latest_version === null ){
				nutils.LOGGER.error( 'Module ' + desired.module + ' is not part of npm.' );
				return callback( '1' );
			}
