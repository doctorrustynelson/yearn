...
module.exports = function( config ){
	...
	ynpm.commands.check = function( desired, callback ){
		...
		if( parts.length === 2 ){
			...
			nutils.getLatestVersionOf( parts[ 1 ], function( err, latest_version ){
				...
				if( err !== null ){
					LOGGER.error( 'Failed to determine latest version of ' + parts + '.' );
					return callback( err );
				}
			...
			LOGGER.error( 'Incorrect number of parts to specify an org/module.' );
			return callback( 1 );
		}
	};
	
	ynpm.commands.npmConfig = function( args, callback ){
		LOGGER.debug( 'npm config ' + args );
	...
	ynpm.commands.install = function( desired, callback ){
		...
		LOGGER.debug( 'Installing ' + desired.module + ' -> ' + tempdir + '.' );
